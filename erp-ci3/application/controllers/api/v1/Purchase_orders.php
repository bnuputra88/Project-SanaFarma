...
class Purchase_orders extends Api_Controller
{
    ...
    public function show(int $id): void
    {
        $this->run(function () use ($id) {
            $this->authorize('purchasing.po.view');
            $this->ok($this->service(Purchase_order_service::class)->get($id));
        });
    }
    ...
    public function action(int $id, string $action): void
    {
        $this->run(function () use ($id, $action) {
            ...
            $status = $this->service(Purchase_order_service::class)->action($id, $action, $this->body()['notes'] ?? null);
